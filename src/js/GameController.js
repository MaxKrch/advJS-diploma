...
export default class GameController {
	...
	createTeam(characters, maxLvl, countCharacters) {
		const team = generateTeam(characters, maxLvl, countCharacters);
		return team;
	}

	calcPositioningTeam(startPosition, fieldSize, boardSize, team) {
	...
	onCellClick(index) {
		...
		const activeCharacter = this.gameState.activeCharacter;
	...
	onCellEnter(index) {
		const character = this.gameState.teams.teamPlayer.find((item) => {
			return item.position == index;
		}) || this.gameState.teams.teamPC.find((item) => {
			return item.position == index;
		});
	...
	initActiveCell(index) {
		...
		if(isPlayer) {
			this.activeCellForChangeCharacter(index);
			return;
		}
	...
	activeCellForChangeCharacter(index) {
	...
	moveCharacter(mover, newCell) {
		...
		team.splice(indexCharacter, 1, mover);
	...
	updateCharacters(damage, attacker, target) {
		...
		if(newHealthTarget <= 0) {
			...
			target.character.health = newHealthTarget;
		}
	...
	addItemForHistoryPoints() {
		...
		const historyPoints = this.gameState.historyPoints;
	...
	onLoadGame() {
		...
		if(state instanceof Error) {
			this.gamePlay.showError(atate.message);
			return;
		}
