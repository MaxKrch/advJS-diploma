export default class GameState {
  ...
  static calcPossibleCellsAttack (index, boardSize, maxStep) {
    ...
    const totalY = (shiftDown - shiftTop) / 8;
  ...
  static shiftAttackTop (index, boardSize, maxStep) {
    ...
    for(let i = index; i >= boardSize; i -= boardSize) {
      maxPossibleShift = maxPossibleShift - boardSize;
    }
  ...
  static shiftAttackRight (index, boardSize, maxStep) {
    ...
    for(let i = 0; i < boardSize; i++) {
      ...
      if(chek === 0) {
        maxPossibleShift = maxPossibleShift + i;
        break;
      }
    }
 ...
 static calcPossibleCellsMove(index, boardSize, maxStep, allowedDirection = {top: true, right: true, down: true, left: true}) {
    const allPossibleRanges = GameState.calcPossibleSteps(index, boardSize, maxStep, allowedDirection);
    const allPossibleCells = allPossibleRanges.map((item) => {
        if(item != 0) {
          return item + index;
        }
      });
...
//добавить player = true и запускать проверку правой ячейки только если
    ...
    const { top, right, down, left } = allowedDirection;
    let chekTop = false;
    let chekDown = false;
    let chekLeft = false;
    let chekRight = false;
